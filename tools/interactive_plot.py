...
def interactive_plot(result, id_list, data_class, xlabel='', ylabel='', title='', color_override=[]):
    ...
    # window_indices_stack_stack = np.empty((0,2))
    # window_indices_stack_stack = np.vstack((window_indices_stack_stack,itm))

    window_indices_stack_stack = np.vstack(window_indices_stack_list)

    print(window_indices_stack_stack.shape)
    print(color_map_stack.shape)
    ...
    color_list = ['gold','royalblue','grey']
    if len(color_override) > 0:
        color_map = []
        for a in color_override:
            color_map.append(color_list[a])
    ...
    try:
        sc = plt.scatter(x, y, c=color_map, s=2, cmap=cmap, norm=norm)
    except:
        sc = plt.scatter(x, y, c='k', s=2, cmap=cmap, norm=norm)

    arr_img = plt.imread("test.png", format='png')
